...
/// * 'auth' The authentication token
/// * 'collection_id' The document path / collection; For example "my_collection" or "a/nested/collection"
pub fn list_async<T, BEARER>(auth: &mut BEARER, collection_id: impl Into<String>) -> AsyncList<T, BEARER>
where
    BEARER: FirebaseAuthBearerAsync,
{
    let collection_id = collection_id.into();
    AsyncList {
        url: firebase_url(auth.project_id(), &collection_id),
...
#[inline]
async fn get_new_data_async<'a>(
    collection_id: &str,
    url: &str,
    auth: &'a mut impl FirebaseAuthBearerAsync,
) -> Result<dto::ListDocumentsResponse> {
    let resp = auth
        .client_async()
        .get(url)
...
/// Please note that this API acts as an iterator of same-like documents.
/// This type is not suitable if you want to list documents of different types.
pub struct AsyncList<'a, T, BEARER> {
    auth: &'a mut BEARER,
    next_page_token: Option<String>,
    documents: Vec<dto::Document>,
    current: usize,
...
impl<'a, T, BEARER> AsyncIterator for AsyncList<'a, T, BEARER>
where
    for<'b> T: Deserialize<'b> + Send,
    BEARER: FirebaseAuthBearerAsync + Sync,
{
    type Item = Result<(T, dto::Document)>;
    ...
    async fn next(&mut self) -> Option<Self::Item> {
        ...
        if self.documents.len() <= self.current {
            ...
            let result = get_new_data_async(&self.collection_id, &url, self.auth).await;
            match result {
                Err(e) => {
                    self.done = true;
                    return Some(Err(e));
