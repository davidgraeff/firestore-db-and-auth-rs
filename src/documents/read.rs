...
pub mod blocking {
    ...
    pub fn read<T>(auth: &impl FirebaseAuthBearer, path: &str, document_id: impl AsRef<str>) -> Result<T>
        where for<'b> T: Deserialize<'b>,
    {
        Ok(auth.rt().block_on(super::read(auth, path, document_id))?)
    }

    /// Return the raw unparsed content of the Firestore document.
    /// See [`super::contents()`]
    pub fn contents(auth: &impl FirebaseAuthBearer, path: &str, document_id: impl AsRef<str>) -> Result<String> {
        Ok(auth.rt().block_on(super::contents(auth, path, document_id))?)
    }
}
